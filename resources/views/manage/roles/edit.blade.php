...
<div class="page-body">
    ...
    <div class="body-content">
        <form class="form-role-new" role="form" action="{{ route('manage.roles.index') }}/{{$role->id}}" accept-charset="UTF-8" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="form-stuf">
                <div class="row">
                    <div class="col-md-8">
                        {{-- <div><b>usename: </b><span>{{ $role->name }}</span></div> --}}
                        @include('manage._includes.html.form-group.name', [
                            'edit_val'=> $role->name,
                            'f_label'=>[
                                'for'=>'name',
                                'text'=>'Name [Alphabets only]'
                                ]
                                ])

                        @include('manage._includes.html.form-group.display_name', [
                                    'edit_val' => $role->display_name,
                                    'f_label'=>[
                                        'for'=>'display_name',
                                        'text'=>'Display Name'
                                    ]
                                    ...
                                    <li>
                                        ...
                                        <ul>
                                            @foreach($permission as $perm)
                                                <li>
                                                    <input type="checkbox" id="permission-{{$perm->id}}" name="permissions[]" class="the-permission" value="{{$perm->id}}" @if(in_array($perm->key, $role->perms->pluck('key')->toArray())) checked @endif>
